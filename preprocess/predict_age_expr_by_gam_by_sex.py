...
def fit_single_gene_by_sex_bak(gene_name, expr_values, covs, X_predicts, cov_num=1):
    # try:
    if cov_num==2:
        gam = LinearGAM(s(0)+f(1)+s(0,by=1) + l(2))
    if cov_num==1:
        gam = LinearGAM(s(0)+f(1)+s(0,by=1))
    gam.fit(covs, expr_values)
    # pprint(gam.terms)
    sex_predict={}
    for k in X_predicts.keys():
        pred, se = predict_with_se(gam,X_predicts[k])
        sex_predict[k]=(pred,se)
    # except:
    #     pred = np.full_like(predicted_ages, np.nan, dtype=float)
    #     se = np.full_like(predicted_ages, np.nan, dtype=float)
    return gene_name, sex_predict

def fit_single_gene_by_sex(gene_name, expr_values, covs, X_predicts, cov_num=1):
    n = 1
    labels = covs[:, n]
    ...
    for k in unique_labels:
        mask = labels == k
        sub_covs = covs[mask]
        sub_expr = expr_values[mask]
        if cov_num == 2:
            gam = LinearGAM(s(0) + l(2))
        if cov_num == 1:
            gam = LinearGAM(s(0))
        gam.fit(sub_covs, sub_expr)
        pred, se = predict_with_se(gam, X_predicts[k])
        sex_predict[k] = (pred, se)
    return gene_name, sex_predict
...
def smooth_expression_by_age_parallel_api_with_se_by_sex(input_path, n_jobs=6, cov_num=1):
    ...
    for k in sex_code.keys():
        init_covs[k]=np.column_stack((age_range,np.full_like(age_range, sex_code[k]),rin_col))
    X_predicts = init_covs
    results = Parallel(n_jobs=n_jobs)(
        delayed(fit_single_gene_by_sex)(
            gene_names[i],
            expr_matrix[i],
            covs,
            X_predicts,
    ...
    sex_predict = {x:([],[]) for x in results[0][1].keys()}
    for r in results:
        pred=r[1]
        for k in pred.keys():
            sex_predict[k][0].append(pred[k][0])
            sex_predict[k][1].append(pred[k][1])
    age_str=[f'{int(ag)}' for ag in age_range]
    return (genes, age_str, sex_predict)


def smooth_expression_by_age_parallel_by_sex(input_path, output_prefix, with_se=['se','mean'], n_jobs=6,cov_num=1):
    make_dir(os.path.dirname(output_prefix))
    genes, predicted_ages, sex_predict = smooth_expression_by_age_parallel_api_with_se_by_sex(input_path,n_jobs,cov_num)
    for k in sex_predict.keys():
        pred_matrix,se_matrix=sex_predict[k]
        ...
        if 'mean' in with_se:
            ...
            result_df.to_csv(f'{output_prefix}-{k}-mean.tsv', sep='\t', index=False, lineterminator='\n', float_format='%.6f')
        if 'se' in with_se:
            ...
            result_df.to_csv(f'{output_prefix}-{k}-se.tsv', sep='\t', index=False, lineterminator='\n', float_format='%.6f')
...
if __name__ == '__main__':
    ...
    for fn in os.listdir(ind_dir):
        log(f'start {fn}')
        for cov_num in range(2,3):
            f_prefix = '.'.join(fn.split('.')[:-1])+f'-cov{cov_num}'
            smooth_expression_by_age_parallel_by_sex(f'{ind_dir}/{fn}',f'{gam_dir}/{f_prefix}',n_jobs=20, cov_num=cov_num)
